    ...
    return new Promise<{ token: string }>((resolve, reject) => {
        const server = Bun.serve({
            ...
            async fetch(req) {
                ...
                if (!code || !state) {
                    settled = true;
                    reject(new Error("Missing cade or state"));
                    setTimeout(() => server.stop(), 500);
                ...
                try {
                    ...
                    reject(err);
                    setTimeout(() => server.stop,500);
                    return new Response("Invlaid state", { status: 400 });
                }
        ...
        authorizeUrl.searchParams.set("client_id", clientId);authorizeUrl.searchParams.set("redirect_uri", redirectUri);
        authorizeUrl.searchParams.set("scope", "openid email profile");
        authorizeUrl.searchParams.set("state", state);authorizeUrl.searchParams.set("prompt", "login");authorizeUrl.searchParams.set("code_challenge", codeChallenge);
