...
import { toolInputSchemas, Mode, type ModeType } from "../../../shared/src/schemas";
...
function resolveInsideCwd(path: string | undefined) {
    ...
    switch (toolName) {
        case "readFile" : {
            ...
            return content.length > MAX_FILE_SIZE
                ? { content: content.slice(0, MAX_FILE_SIZE), truncated: true, totalLength: content.length} : { content };
        }
        case "listDirectory" : {
