...
export function createGrepTool(cwd: string) {
    return tool({
        description:
            "Search file contents using a regex pattern. Return matching lines with file paths and line numbers. skip hidden directories, node_modules, and binary files",
        ...
        execute: async ({ pattern, path, include }) => {
            ...
            try{
                ...
                return {
                    matches,
                    ...Bun(truncated ? { truncated: true, totalMatches: lines.length } : {}),
                };
